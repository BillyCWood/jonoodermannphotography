...
const Header = () => {
  ...
  return (
    ...
    <header className={clsx(
        ...
        hasScrolled && 'py-2 bg-jop-green shadow-md shadow-black backdrop-blur-[8px]',
        )}>
        <div className='lg:hidden w-full pl-10 z-2'>
            <div className='flex w-full justify-between items-center'>
                ...
                <button className='justify-center items-center w-10 overflow-hidden mr-10' onClick={() => setIsOpen((prevState) => !prevState)}>
                    <img src={`/images/${isOpen ? 'close-' : 'hamburger-'}white.svg`} className='size-3/4' />
                </button>
            </div>
            <div className={`${isOpen ? 'flex flex-col items-end float-right right-0 top-12 w-2/5 text-white' : 'hidden'}`}>
